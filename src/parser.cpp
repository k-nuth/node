...
config::checkpoint::list parser::default_checkpoints() {
    ...
    // EBP testnet
    if (configured.network.identifier == 2811197310u) {
        checkpoints.reserve(1);
        checkpoints.emplace_back("00000000a437e07f6bd52b7fe96f75688593506f2726c19c3a2383c278b90052", 0);
    } else {
...
void parser::fix_checkpoints() {
    ...
    auto const it = std::max_element(checkpoints.begin(), checkpoints.end(), [](checkpoint const& x, checkpoint const& y) {
    ...
    if (it != checkpoints.end()) {
        ...
        configured.chain.checkpoints.erase(std::remove_if(configured.chain.checkpoints.begin(), configured.chain.checkpoints.end(), [max_checkpoint_height](checkpoint const& x) {
