...
protocol_header_in::protocol_header_in(full_node& node, channel::ptr channel, safe_chain& chain)
...
void protocol_header_in::start() {
    protocol_timer::start(header_latency_, [this](code const& ec){
        handle_timeout(ec);
    });
...
void protocol_header_in::send_top_get_headers(const hash_digest& stop_hash) {
    auto const heights = block::locator_heights(node_.top_header().height());

    chain_.fetch_header_locator(heights, [this, &stop_hash](code const& ec, get_headers_ptr message){
        handle_fetch_header_locator(ec, message, stop_hash);
    });
}

void protocol_header_in::handle_fetch_header_locator(code const& ec, get_headers_ptr message, hash_digest const& stop_hash) {
    if (stopped(ec)) {
        return;
    }

    if (ec) {
        LOG_ERROR(LOG_NODE, "Internal failure generating header locator for [", authority(), "] ", ec.message());
        stop(ec);
        return;
    }
    ...
    if (stop_hash == null_hash) {
        LOG_DEBUG(LOG_NODE, "Ask [", authority(), "] for headers after [", encode_hash(last_hash), "]");
    } else {
        LOG_DEBUG(LOG_NODE, "Ask [", authority(), "] for headers from [", encode_hash(last_hash), "] through [", encode_hash(stop_hash), "]");
    }
...
// TODO: move headers to a derived class protocol_header_in_31800.
...
bool protocol_header_in::handle_receive_headers(code const& ec, headers_const_ptr message) {
    if (stopped(ec)) {
        return false;
    }
...
bool protocol_header_in::handle_receive_inventory(code const& ec, inventory_const_ptr message) {
    if (stopped(ec)) {
        return false;
    }
...
void protocol_header_in::send_get_data(code const& ec, get_data_ptr message) {
    if (stopped(ec)) {
        return;
    }
...
// TODO: move not_found to a derived class protocol_header_in_70001.
bool protocol_header_in::handle_receive_not_found(code const& ec, not_found_const_ptr message) {
    if (stopped(ec)) {
        return false;
    }
...
void protocol_header_in::organize_block(block_const_ptr message) {
    message->validation.originator = nonce();
    chain_.organize(message, BIND2(handle_store_block, _1, message));
}

bool protocol_header_in::handle_receive_block(code const& ec, block_const_ptr message) {
    if (stopped(ec)) {
        return false;
    }
...
bool protocol_header_in::handle_receive_block_transactions(code const& ec, block_transactions_const_ptr message) {
    if (stopped(ec)) {
        return false;
    }
...
void protocol_header_in::handle_fetch_block_locator_compact_block(code const& ec, get_headers_ptr message, hash_digest const& stop_hash) {
    if (stopped(ec)) {
        return;
    }
...
bool protocol_header_in::handle_receive_compact_block(code const& ec, compact_block_const_ptr message) {
    if (stopped(ec)) {
        return false;
    }
...
void protocol_header_in::send_get_data_compact_block(code const& ec, hash_digest const& hash) {
    hash_list hashes;
...
void protocol_header_in::handle_store_block(code const& ec, block_const_ptr message) {
    if (stopped(ec)) {
        return;
    }
...
void protocol_header_in::handle_timeout(code const& ec) {
    if (stopped(ec)) {
...
void protocol_header_in::handle_stop(code const&) {
    LOG_DEBUG(LOG_NETWORK, "Stopped header_in protocol for [", authority(), "].");
}
...
#if defined(KTH_STATISTICS_ENABLED)
void protocol_header_in::report(domain::chain::block const& block, full_node& node) {
#else
void protocol_header_in::report(domain::chain::block const& block) {
#endif
