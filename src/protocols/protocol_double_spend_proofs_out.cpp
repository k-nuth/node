...
#define NAME "double_spend_proofs_out"
#define CLASS protocol_double_spend_proofs_out
...
protocol_double_spend_proofs_out::protocol_double_spend_proofs_out(full_node& node, channel::ptr channel, safe_chain& chain)
    : protocol_events(node, channel, NAME)
    , chain_(chain)
    , ds_proofs_enabled_(node.node_settings().ds_proofs_enabled)
    , CONSTRUCT_TRACK(protocol_double_spend_proofs_out)
{}

// Start.
//-----------------------------------------------------------------------------

void protocol_double_spend_proofs_out::start() {
    protocol_events::start(BIND1(handle_stop, _1));

    // // TODO: move relay to a derived class protocol_double_spend_proofs_out_70001.
    ...
    // // TODO: move fee filter to a derived class protocol_double_spend_proofs_out_70013.
...
// // TODO: move fee_filters to a derived class protocol_double_spend_proofs_out_70013.
// bool protocol_double_spend_proofs_out::handle_receive_fee_filter(code const& ec, fee_filter_const_ptr message) {
//     if (stopped(ec)) {
//         return false;
//     }
...
// // TODO: move memory_pool to a derived class protocol_double_spend_proofs_out_60002.
// bool protocol_double_spend_proofs_out::handle_receive_memory_pool(code const& ec, memory_pool_const_ptr) {
//     if (stopped(ec)) {
//         return false;
//     }

//     // // The handler may be invoked *multiple times* by one blockchain call.
//     // // TODO: move multiple mempool inv to protocol_double_spend_proofs_out_70002.
//     // // TODO: move fee filter to a derived class protocol_double_spend_proofs_out_70013.
...
// void protocol_double_spend_proofs_out::handle_fetch_mempool(code const& ec, inventory_ptr message) {
...
// bool protocol_double_spend_proofs_out::handle_receive_get_data(code const& ec, get_data_const_ptr message) {
//     if (stopped(ec)) {
//         return false;
//     }
...
// void protocol_double_spend_proofs_out::send_next_data(inventory_ptr inventory) {
...
// void protocol_double_spend_proofs_out::send_transaction(code const& ec, transaction_const_ptr message, size_t position, size_t /*height*/, inventory_ptr inventory) {
//     if (stopped(ec)) {
//         return;
//     }
...
// void protocol_double_spend_proofs_out::handle_send_next(code const& ec, inventory_ptr inventory) {
//     if (stopped(ec)) {
//         return;
//     }
...
// bool protocol_double_spend_proofs_out::handle_transaction_pool(code const& ec, transaction_const_ptr message) {
//     if (stopped(ec)) {
//         return false;
//     }
...
//     // // TODO: move fee_filter to a derived class protocol_double_spend_proofs_out_70013.
//     // if (message->fees() < minimum_peer_fee_) {
//     //     return true;
//     // }

//     inventory::type_id id = inventory::type_id::transaction;
...
void protocol_double_spend_proofs_out::handle_stop(code const&) {
    chain_.unsubscribe();

    LOG_DEBUG(LOG_NETWORK, "Stopped double_spend_proofs_out protocol for [", authority(), "].");
}
